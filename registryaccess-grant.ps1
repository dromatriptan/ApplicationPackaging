...
function GrantRegistryAccess {
    ...
    if ( (Test-Path -Path $keyName -PathType Container) -eq $true ) {
        ...
        $regRule = New-Object System.Security.AccessControl.RegistryAccessRule ($idRef,$permission,$inhFlags, $prFlags, $acType)

        $regAcl = Get-Acl -Path $keyName
        $regAcl.AddAccessRule($regRule)
        $regAcl.SetAccessRule($regRule)
        $regAcl | Set-Acl -Path $keyName
...
$registryPermissioned = GrantRegistryAccess -groupName $groupName -keyName $keyName -permission ChangePermissions
<# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ Main Program ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ #>